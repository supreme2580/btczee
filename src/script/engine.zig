...
const stack = @import("opcodes/stack.zig");
...
pub const Engine = struct {
    ...
    fn executeOpcode(self: *Engine, opcode: u8) !void {
        ...
        try switch (opcode) {
            ...
            0x6b => try stack.opcodeToAltStack(self),
            0x6c => try stack.opcodeFromAltStack(self),
            0x6d => try self.op2Drop(),
            0x6e => try self.op2Dup(),
            0x6f => try self.op3Dup(),
            0x70 => try stack.opcode2Over(self),
            0x71 => try stack.opcode2Rot(self),
            0x72 => try stack.opcode2Swap(self),
            0x73 => try self.opIfDup(),
            0x74 => try self.opDepth(),
            0x75 => try self.opDrop(),
            0x76 => try self.opDup(),
            0x77 => try stack.opcodeNip(self),
            0x78 => try stack.opcodeOver(self),
            0x79 => try stack.opcodePick(self),
            0x7a => try stack.opcodeRoll(self),
            0x7b => try stack.opcodeRot(self),
            0x7c => try stack.opcodeSwap(self),
            0x7d => try stack.opcodeTuck(self),
